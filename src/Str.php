...
class Str
{
     ...
     * @return mixed|string
     */
    public static function get_online_ip()
    {
        return Curl::get('http://ipinfo.io/json')['ip'] ?? '';
    }
     ...
     * @return string
     */
    public static function get_user_agent(string $agent = ''): string
    {
        if(empty($agent)) return 'Unknown';

        if (preg_match('/Windows NT|Win32|Mac|Macintosh|MacIntel|Mac OS X|Linux|Ubuntu/i', $agent)) {
            return 'PC';
     ...
     * @return array
     */
    public static function split_utf8($str)
    {
        $split = 1;
        $array = array();
     ...
     * 中文分割字符串到数组
     * @param string $string
     * @param int $split_length 多少个字符分割为一组
    ...
    public static function mb_width_split(string $string, int $font_size, int $split_width, string $encoding = null)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            // 获取当前字符的宽度
            $char_w = imagefontwidth($font_size) * mb_strwidth($char);

            if ($cur_l + $char_w <= $split_width) {
                $line .= $char;
                $cur_l += $char_w;
            }else{  // 断行
                $parts[] = $line;
    ...
    public static function is_html_tag($string) {
        return 0 !== preg_match('/^<\/?(?!.*script)[^>]+>$/', $string);
    }
    ...
    public static function is_mobile($string){
        return 0 !== preg_match('/^1[3-9][0-9]\d{8}$/', $string);
    }

    /**
     * 获取手机号基础运营商（过滤虚拟号段）
     * @param string $mobile 11位手机号码
     * @return string 运营商名称/类型
     */
    public static function get_mobile_server($mobile) {
        // 虚拟运营商号段
        $virtualPrefix = [
            '1700', '1701', '1702', '1703', '1705', '1706',
            '1710', '1711', '1713', '1715', '1716', '1718',
            '1670', '1671', '1672', '1673', '1674', '1675',
            '1620', '1650', '1651', '1652', '1653', '1600'
        ];

        // 优先检测4位虚拟号段
        $prefix4 = substr($mobile, 0, 4);
        if (in_array($prefix4, $virtualPrefix)) {
            return '虚拟运营商';
        }

        // 检测3位虚拟号段
        $prefix3 = substr($mobile, 0, 3);
        if (in_array($prefix3, ['170', '171', '167', '162', '165'])) {
            return '虚拟运营商';
        }
        ...
        $operatorMap = [
            '移动' => [
                '1340', '1350', '1360', '1370', '1380', '1390', '1440',
                '1470', '1480', '1500', '1510', '1520', '1570', '1580',
                '1590', '1720', '1780', '1820', '1830', '1840', '1870',
                '1880', '1950', '1980', '1973', '1974'
            ],
            '联通' => [
                '1300', '1310', '1320', '1550', '1560', '1660', '1750',
                '1760', '1850', '1860', '1960', '1961', '1962', '1963'
            ],
            '电信' => [
                '1330', '1490', '1530', '1730', '1770', '1800', '1810',
                '1890', '1900', '1910', '1930', '1990', '1991', '1992',
                '1740', '1741', '1742'
            ]
        ];

        // 优先匹配4位精确号段
        foreach ($operatorMap as $operator => $prefixes) {
            if (in_array($prefix4, $prefixes)) {
                return $operator;
            }
        }
        ...
        $threeDigitMap = [
            ...
            '144' => '移动', '147' => '移动', '148' => '移动',
            '150' => '移动', '151' => '移动', '152' => '移动',
            '157' => '移动', '158' => '移动', '159' => '移动',
            '172' => '移动', '178' => '移动', '182' => '移动',
            '183' => '移动', '184' => '移动', '187' => '移动',
            '188' => '移动', '195' => '移动', '198' => '移动',

            '130' => '联通', '131' => '联通', '132' => '联通',
            '155' => '联通', '156' => '联通', '166' => '联通',
            '175' => '联通', '176' => '联通', '185' => '联通',
            '186' => '联通', '196' => '联通',

            '133' => '电信', '149' => '电信', '153' => '电信',
            '173' => '电信', '177' => '电信', '180' => '电信',
            '181' => '电信', '189' => '电信', '190' => '电信',
            '191' => '电信', '193' => '电信', '199' => '电信'
        ];

        if (isset($threeDigitMap[$prefix3])) {
            return $threeDigitMap[$prefix3];
        }

        // 处理特殊号段
        if (preg_match('/^19[1-9]/', $mobile)) {
            return '物联网号段';
        }
     ...
     * 对字符串参数过滤
     * @param string $str
     * @param bool $allow_link 是否允许有链接
     * @return string
     */
    public static function str_filter(string $str, bool $allow_link = false): string
    {
        //用户输入都是不靠谱的！
        if (!$allow_link && preg_match('/https?:\/\/[\w.]+[\w\/]*[\w.]*\??[\w=&\+\%]*/is', $str)) return '';
...
//            '.',
        ...
        return trim(str_replace($filter_arr, '', $str));
    }
     ...
     */
    public static function del_enter($str){
        return preg_replace("/[\t\n\r]+/", '', $str);
    }
     ...
     * @return string
     */
    public static function msubstr(string $str, $start, $length, $charset = "utf-8", $suffix = true)
    {
     ...
     * @param  Str  $type  字串类型
     * 0 字母 1 数字 其它 混合
     * @param  Str  $addChars  额外字符
     * @return Str|false|string
     */
    ...
    public static function build_format_rand($format, $number = 1)
    {
        ...
        for ($j = 0; $j < $number; $j++) {
            ...
            for ($i = 0; $i < $length; $i++) {
                ...
                switch ($char) {
                    case "*"://字母和数字混合
                        $strtemp .= Str::rand_string(1);
                        break;
                    case "#"://数字
                        $strtemp .= Str::rand_string(1, 1);
                        break;
                    case "$"://大写字母
                        $strtemp .= Str::rand_string(1, 2);
                        break;
                    default://其他格式均不转换
                        $strtemp .= $char;
                        break;
     ...
     * @param string $left
     * @param string $right
     * @param string $ignore  查找结果中需删去的字符
     * @param string $mode  all-全部匹配  once-只匹配一次
     * @return array|array[]|mixed|string|string[]|\string[][]
     */
    public static function sub_between($string, $left, $right, $ignore = '', $mode = 'all')
    {
        if(empty($string)) return '';

        $matches = [];
        $pattern = "/(?<=$left).*?(?=$right)/";  // 非贪婪匹配 .*?
        ...
        if($mode == 'all'){
            ...
            if(!empty($matches)){
                $matches = array_map(function ($match) use($ignore) {
                    return str_replace($ignore, '', $match);
                }, $matches);
            }
        ...
        if($mode == 'once'){
            ...
            return !empty($matches) ? str_replace($ignore, '', $matches[0]) : '';
        }
     ...
     * 只执行一次的字符串替换
     * User: root
     * DateTime: 2021/9/1 11:16
     * @param $str
     * @param $find
     * @param  string  $search
     * @return string
     */
    public static function replace_once($str, $find, $replace = '')
    {
        $before = strstr($str, $find, true);
        $after = substr(strstr($str, $find), strlen($find));
     ...
     * @param $str
     * @param $find
     * @param  array  $search_arr 一维数组
     * @param  bool  $is_quotes 字符串类型是否添加分号
     * @return string
     */
    public static function replace_array($str, $find, $search_arr = [], $is_quotes = false)
    {
        if(!$str) return '';

        if (empty($search_arr)) return $str;

        foreach ($search_arr as &$search) {
            $search = ($is_quotes && is_string($search)) ? '\''.$search.'\'' : $search;
        }

        $str = str_replace($find, '%s', $str);
        return vsprintf($str, $search_arr);
    }
     ...
     * @return mixed|string
     */
    public static function parse_camel($string, $ucfirst = false)
    {
     ...
     * @return string
     */
    public static function parse_underline($string)
    {
     ...
     * 去除重复句子
     * User: root
     * DateTime: 2021/9/2 9:43
     * @param $content
     * @return string
     */
    public static function unqiue_article($content)
    {
        ...
        while (!empty($content)) {
            $one_str = mb_substr($content, 0, 1);
            $content = mb_substr($content, 1);
            $new_content .= $one_str;

            if (self::is_mb($one_str)) {
                $del_str .= $one_str;
                continue;
            }
            ...
            // 搜索是否有重复的句子,有就删除
            while (false !== mb_strpos($content, $del_str)) {
                $index = mb_strpos($content, $del_str);
                $str_before = mb_strstr($content, $del_str, true);
                $str_after = mb_substr($content, $index + mb_strlen($del_str));

                if ($index == 0 || !self::is_mb(mb_substr($content, $index - 1, 1))) {
                    $content = $str_before.mb_substr($str_after, 1);
                } else {
                    $content = $str_before.'#replace#'.$str_after;
                }
            }

            $content = str_replace('#replace#', $del_str, $content);
     ...
     * 是否是汉字（不包含标点符号）
     * User: root
     * DateTime: 2021/7/23 9:48
     * @param $str
     * @return 0|1
     */
    public static function is_mb($str)
    {
    ...
    /**
     * 获取批量更新sql语句
     * @param string $table_name 数据表名(带前缀)
     * @param array $data 数据格式 [id1 => [单条数据1], id2 => [单条数据2]...]
     * @param string $primary_key_name 主键名称
     * @param string $primary_key_type 主键数据类型,可为int/string,默认为int
     *
     * @return string
     */
    public static function get_multi_udp_sql($table_name, $data, $primary_key_name = 'id', $primary_key_type = 'int')
    {
        if (empty($data)) {
            return '';
        }
        ...
        $map = [];  // 转换 $data 为 [field => id => value]，防止 $data 每一条的字段内容不一致
        $ids = [];

        array_walk($data, function ($item, $id) use(&$map, &$ids) {
            if(!empty($id)){  // id必须是有效值
                $ids[] = $id;
        ...
        //合成sql语句
        array_walk($map, function ($item, $field)use(
            &$sql,
            $primary_key_name,
            $primary_key_type
        ){
            $sql .= "`$field` = CASE `$primary_key_name` ";
            ...
            foreach($item as $id => $value){
                ...
                if (is_string($value)) {
                    $field_val = str_replace("'", "''", $value);
                    $sql .= "WHEN $id THEN '$field_val' ";
                } elseif (is_numeric($value)) {
                    $sql .= "WHEN $id THEN $value ";
                }
            }

            $sql .= "ELSE `$field` END, ";  // 未匹配到when就保留原值
        });
        ...
        //拼接sql
        $sql .= " WHERE `{$primary_key_name}` IN ({$ids})";
     ...
     * @param string $unique_key 唯一约束的字段
     * @return string
     */
    public static function get_multi_insert_sql(string $table_name, array $data, string $unique_key = ''): string
    {
        //拼接批量更新sql语句
        $sql = "INSERT INTO `{$table_name}` (`";
        $field_arr = array_keys(current($data));
        $sql .= implode('`, `', $field_arr);
        $sql .= '`)  VALUES ';
        $tmp = array_pad([], count($field_arr), '?');  // 有几个字段，就填充几个?
        $tmp = '(' . implode(', ', $tmp) . ')';

        foreach($data as $item){
            $values = array_values($item);
            $sql .= self::replace_array($tmp, '?', $values, true) . ',';
        }

        if(!empty($unique_key)){  //  处理唯一约束的字段,值存在就变为update
            $sql .= rtrim($sql, ',') . ' ON DUPLICATE KEY UPDATE ';

            foreach($field_arr as $field){
                if($field != $unique_key) $sql .= "`$field` = VALUES(`$field`),";
            }
     ...
     * @return string
     */
    public static function get_deep_page_sql($table_name, $offset, $size, $field = '*', $pk = 'id', $where = ''){
        $where  = $where ?: '1';
        ...
        $sql = <<<SQL
            SELECT $field
                    FROM $table_name t 
            INNER JOIN (
               SELECT `$pk` as `j_$pk` FROM `$table_name` WHERE $where LIMIT $offset,$size 
            ) p ON t.`$pk` = p.`j_$pk`
SQL;

        return self::del_enter($sql);
    }
     ...
     * @return string
     */
    public static function get_deep_tree_sql(string $table_name, $field = '*', string $id = 'id', string $pid = 'pid')
    {
        return "select $field from $table_name a where not exists (
		select $id from $table_name b where b.$pid = a.$id)";
    }

    /**
     * 获取子域名
     * @param string $url
     * @param bool $with_scheme 是否带协议头（http/https）
     * @return false|mixed|string
     */
    public static function get_sub_domain(string $url, bool $with_scheme = false){
        ...
        if (count($domain_parts) > 2) {
            ...
            if($with_scheme){
                ...
                $sub_domain = strstr(parse_url($url, PHP_URL_HOST), '.', true);
            }
     ...
     * @return mixed|string
     */
    public static function get_uri(string $url){
        $url_info = parse_url($url);
        $host = $url_info['host'];
        isset($url_info['port']) && ($host .= ':' . $url_info['port']);
     ...
     * @return string             拼音字符串
     */
    public static function get_pinyin($str, $ret_format = 'all', $placeholder = '_', $allow_chars = '/[a-zA-Z\d ]/') {
        static $pinyins = null;
    ...
    /**
     * 整数转excel列序号，1 => A, 26 => Z, 27 => AA
     * @param int $number
     * @return string
     */
    public static function int_to_excel_col(int $number): string {
        if ($number < 1) return '';
     ...
     */
    public static function html_reload($sleep_time = 1){
        sleep($sleep_time);
    ...
    {
        echo '<pre style=font-size:14px;color:#00ae19;>';
        print_r($data);
        echo '</pre>';
    }
     ...
     * @return string
     */
    public static function json_to_db(string $json)
    {
